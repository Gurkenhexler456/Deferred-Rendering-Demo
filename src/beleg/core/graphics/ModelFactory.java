...
public class ModelFactory {
	...
	public static Model buildDefaultModel(Mesh _mesh) {
	...
	public static Model buildModel(Mesh _mesh, VertexBufferLayout[] _layout) {
		...
		for(int i = 0; i < _layout.length; i++) {
			...
			GL20.glVertexAttribPointer(	i, 	
										_layout[i].getCount(), 
										_layout[i].getType(), 
										_layout[i].getNormalized(), 
										_layout[i].getStride(), 
										offset);
			
			offset = _layout[i].getCount() * _layout[i].getElementSize();
		}
		GL15.glBindBuffer(GL15.GL_ARRAY_BUFFER, 0);
